...
use <?= ltrim($generator->modelClass, '\\') ?>Form;
 ...
 */
abstract class <?= $controllerClass ?> extends <?= StringHelper::basename($generator->baseControllerClass)."\n" ?>
{
<?php
...
<?php endif; ?>
    ...
    {
<?php if ($searchModelClass !== ''): ?>
        $searchModel  = new <?= $searchModelClassName.($softdelete?"(['is_deleted' => FALSE])":"") ?>;
...
<?php if ($softdelete): ?>
    /**
     * Lists all archive of <?= $modelClass ?> models.
     * @return mixed
     */
...
?>
    ...
    /**
     * Finds the <?= $modelClass ?>Form model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * <?= implode("\n     * ", $actionParamComments)."\n" ?>
     * @return <?= $modelClass ?> the loaded model
     * @throws HttpException if the model cannot be found
